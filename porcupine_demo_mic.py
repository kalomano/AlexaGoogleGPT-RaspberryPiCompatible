...
class PorcupineDemo(Thread):
    ...
    def run(self):
        ...
        try:
            ...
            stream = audioPy.open(format=pyaudio.paInt16, channels=1, rate=33100, input=True,
                                frames_per_buffer=1024)
            fileName = "grab.mp3"
            filePath = ""
            
            #clientAPIGoogleSpeech
            client = speech.SpeechClient.from_service_account_file(filePath+"key.json")
            ...
            while True:#Hearing audio loop (always)

                pcm = recorder.read()#read Porcupine

                if (recording==True):#read pyAudio (if is recording the audio)
                ...
                if result >= 0:
                    ...
                    if keywords[result] == "alexa":
                        # Alexa means START recording (pyAudio)
                        print("Recording")
                        
                        #play sound login, with PyGame
                        pygame.mixer.init()
                        pygame.mixer.music.load(filePath+"login.mp3")
                        ...
                        # Blueberry means STOP recording (pyAudio)
                        print("Audio grabado")
                        
                        #play sound logout, with PyGame
                        pygame.mixer.init()
                        pygame.mixer.music.load(filePath+"logout.mp3")
                        pygame.mixer.music.play()
                        ...
                        #and save audio from pyAudio recording
                        sound_file = wave.open((filePath+fileName), "wb")
                        sound_file.setnchannels(1)
                        sound_file.setsampwidth(audioPy.get_sample_size(pyaudio.paInt16))
                        sound_file.setframerate(44100)
                        sound_file.writeframes(b''.join(frames))
                        sound_file.close()
                        recording = False

                        #when audio is recorded speechToText with GoogleAPI
                        ...
                        responseGoogle = client.recognize(
                            config=config,
                            audio=audioFile
                        )
                        try:
                            print(responseGoogle.results[0].alternatives[0].transcript)

                            #get OpenAIApiGPT answer
                            responseOpenAI = openai.Completion.create(
                                model="text-babbage-001",
                                prompt="Answer me with no much words: "+(responseGoogle.results[0].alternatives[0].transcript + "\n"),
                                temperature=0.7,
                                max_tokens=100,
                                top_p=1,
                                frequency_penalty=0,
                            ...
                            print(responseOpenAI.choices[0].text)

                            #text to Speech with GTTS
                            audioGTTS = gTTS(text=responseOpenAI.choices[0].text, lang="en", slow=False)
                            audioGTTS.save(filePath+"answer.mp3")
                            
                            #play sound saved, with PyGame
                            pygame.mixer.init()
                            pygame.mixer.music.load(filePath+"answer.mp3")
                            pygame.mixer.music.play()
